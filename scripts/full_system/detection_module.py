...
genai.configure(api_key='AIzaSyCn3xz6FWTR7KCrcTRPojjxvMTHheBUkeQ')
tracker_llm = genai.GenerativeModel('gemini-2.0-flash')
...
def get_object_tags(img_pil):
    ...
    """
    response = tracker_llm.generate_content([
        img_pil,
        (
...
#                                         key=lambda x: x['depth_distance'] if x['depth_distance'] is not None else float(
#                                             'inf'))
#                 for i, obj in enumerate(sorted_results[:3]):
#                     if obj["depth_distance"] is not None:
#                         print(
#                             f"目标 {i + 1}: {obj['label']} - 距离: {obj['depth_distance']:.2f}m, 角度: {obj['yaw_angle']:.1f}°")
...
#                                     'get_airsim_scene': lambda: get_airsim_scene(),
#                                     'get_depth_images': lambda: get_depth_images()
#                                 }
#                                 # 执行代码
...
def process_scene_with_detection():
    ...
    if boxes:
        # 传递图像尺寸作为scene_shape
        depth_results = estimate_depth_yaw(boxes, labels, img_pil.size)
...
#     response = tracker_llm.generate_content([prompt], tools={"function_declarations": tracker_tool})
#
#     # 检查Gemini响应，调用实际函数
